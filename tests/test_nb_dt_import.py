...
def test_filter_vendors_for_parsed_types_uses_parsed_subset():
    nb_dt_import = load_nb_dt_import_module()
...
def test_log_run_mode_reports_update_and_remove_enabled():
    nb_dt_import = load_nb_dt_import_module()
    handle = MagicMock()
...
def test_has_missing_device_images_detects_image_changes():
    nb_dt_import = load_nb_dt_import_module()
...
def test_select_device_types_for_update_mode_scopes_to_new_and_modified():
    nb_dt_import = load_nb_dt_import_module()
    device_types = [
        {"manufacturer": {"slug": "cisco"}, "model": "A", "slug": "a"},
        {"manufacturer": {"slug": "cisco"}, "model": "B", "slug": "b"},
...
def test_items_per_second_column_handles_empty_speed():
    nb_dt_import = load_nb_dt_import_module()
    column = nb_dt_import.ItemsPerSecondColumn()

    rendered = column.render(SimpleNamespace(finished=False, speed=None, finished_speed=None))

    assert str(rendered) == "- it/s"


def test_items_per_second_column_renders_speed_value():
    nb_dt_import = load_nb_dt_import_module()
    column = nb_dt_import.ItemsPerSecondColumn()

    rendered = column.render(SimpleNamespace(finished=False, speed=12.34, finished_speed=None))

    assert str(rendered) == "12.3 it/s"


def test_items_per_second_column_uses_elapsed_fallback():
    nb_dt_import = load_nb_dt_import_module()
    column = nb_dt_import.ItemsPerSecondColumn()
